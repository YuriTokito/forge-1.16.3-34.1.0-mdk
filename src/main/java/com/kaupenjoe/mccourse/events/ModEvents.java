...
{
    @SubscribeEvent
    public void unCopperedSheep(AttackEntityEvent event)
    {
    ...
    @SubscribeEvent
    public void dontHitTheParrot(AttackEntityEvent event)
    {
        if(event.getTarget().isAlive())
        {
            LivingEntity target = (LivingEntity) event.getTarget();
            if (target instanceof ParrotEntity)
            {
                ...
                player.addPotionEffect(new EffectInstance(Effects.INSTANT_DAMAGE, 10));

                if (!player.world.isRemote())
                {
                    String message = TextFormatting.DARK_PURPLE + "Oi! Don't hit the parrots or the pirates will come!";
